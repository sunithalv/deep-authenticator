...
class UserLoginEmbeddingValidation:
    ...
    def generate_embedding_list(files: List[Bytes]) -> List[np.ndarray]:
        ...
        try:
            embedding_list = []
            #For multiple images uploaded by the use gerenerate the mebeddings as a list
            for contents in files:
                img = Image.open(io.BytesIO(contents))
                # read image array
                img_array = np.array(img)
                # Detect face from the image and generate embedding for the same
    ...
    def cosine_simmilarity(db_embedding, current_embedding) -> bool:
        ...
        """
        try:
            #Find dot product of embeddings/product of normalized embeddings
    ...
    def compare_embedding(self, files: bytes) -> bool:
        ...
        try:
            ...
            if self.user:
                ...
                similarity = UserLoginEmbeddingValidation.cosine_simmilarity(
